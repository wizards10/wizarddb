...
class ICommand
{
    typedef int (*OnMsgBuild) (char** ppBuffer , int* pBufferSize, \
                bson::BSONObj& obj);
public:
    virtual int execute(_ossSocket& sock , std::vector<std::string>& argVec);
    int         getError(int code);
protected:
    int        recvReply(_ossSocket& sock);
    int        sendOrder(_ossSocket& sock, OnMsgBuild onmsgBuild);
    int        sendOrder(_ossSocket& sock, int opCode);
protected:
    virtual int handleReply() 
        {
            return EDB_OK;
        }
protected:
...
{
public:
    int execute(_ossSocket& sock , std::vector<std::string>& argVec);
private:
    std::string _address;
    int         _port;
};
