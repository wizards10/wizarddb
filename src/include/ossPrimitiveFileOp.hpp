...
public:
    ...
    void Close();
    bool isValid(void);
    int Read(const size_t size , void* const pBuf , int* const pBytestRead);
    int Write(const void* pBuf , size_t len = 0);
    int fWrite(const char* fmt , ...);
    offsetType getCurrentOffest(void) const;
    void seekToOffest(offsetType offset);
    void seekToEnd(void);
