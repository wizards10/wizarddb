...
_ossSocket::_ossSocket(int* sock , int timeout)
{
    ...
    if(rc)
    {
        printf("Failed to get sock name , errno = %d", SOCKET_GETLASTERROR);
    }
    else
    {
        ...
        if(rc)
        {
            printf("Failed to get name ,errno = %d",SOCKET_GETLASTERROR);
        }
    }
}
...
int _ossSocket::send(const char* pMsg , int len , int timeout , int flags)
{
    ...
    while(true)
    {
        ...
        if(rc < 0)
        {
            ...
            printf("Failed to select from socket = %d",rc);
            rc = EDB_NETWORK;
            goto error;
        }
...
int _ossSocket::connect()
{
    ...
    if(rc)
    {
        printf("Failed to get peer adderss , rc = %d",rc);
        rc = EDB_NETWORK;
        goto error;
    }
done:
